...
wp_sites                = { 'www.site1.de': 'user1@gmail.com',
                            'www.site2.de': 'user1@gmail.com',
                            'http://www.site3.org': 'user2@gmail.com',
                            'http://www.site5.com': 'user2@gmail.com' }

# Adjust the false positive strings to you needs
# These are useful values from my demo sites
false_positive_strings  = [ 'XML-RPC', 'GHOST vulnerability', 'file exists exposing a version number',
                            'https://wpvulndb.com/vulnerabilities/7358' ]

# Log file
log_file                = r'./wpwatcher.log' # will be written to wpscan dir because we change the working dir

# Email Report
send_email_report       = True
smtp_server             = r'mailserver.de:587' # 'server:port'
smtp_auth               = True
smtp_user               = 'office'
smtp_pass               = 'password'
smtp_ssl                = True
from_email              = r'wpwatcher@office.de'
...
def is_wpscan_installed():
    ...
    if not os.path.exists(wpscan_dir):
        result = subprocess.Popen(['mkdir', '-p', wpscan_dir], stdout=subprocess.PIPE).communicate()[0]
...
def update_wpscan():
    ...
    os.chdir(wpscan_dir)
    try:
        result = subprocess.Popen(['./wpscan.rb', '--batch', '--update'], stdout=subprocess.PIPE).communicate()[0]
...
def run_scan():
    ...
    for wp_site in wp_sites:
        ...
        try:
            ...
            result = subprocess.Popen(['./wpscan.rb', '--batch', '--follow-redirection', '--no-color', '--url', wp_site], stdout=subprocess.PIPE).communicate()[0]
        ...
        # Report Options ------------------------------------------------------
        # Email
        if send_email_report and ( warnings or alerts ):
